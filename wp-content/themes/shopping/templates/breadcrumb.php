...
<?php
        ...
        <?php
		        ...
		        if ( is_category() ) {
			        ...
			        echo $before . __('Archive by date "', TEXTDOMAIN) . get_the_time('d') . '"' . $after;
		        } elseif ( is_month() ) {
			        echo '<a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a> ' . $delimiter . ' ';
			        echo $before . __('Archive by month "', TEXTDOMAIN) . get_the_time('F') . '"' . $after;
		        } elseif ( is_year() ) {
		        	echo $before . __('Archive by year "', TEXTDOMAIN ) . get_the_time('Y') . '"' . $after;
		            ...
		            echo $before . __('Search results for "', TEXTDOMAIN) . get_search_query() . '"' . $after;
		        } elseif ( is_tag() ) {
		        	echo $before . __('Archive by tag "', TEXTDOMAIN) . single_tag_title('', false) . '"' . $after;
		        } elseif ( is_author() ) {
		        global $author;
		        $userdata = get_userdata($author);
		        	echo $before . __('Articles posted by "', TEXTDOMAIN) . $userdata->display_name . '"' . $after;
		        } elseif ( is_404() ) {
